      ...
      <section className="max-w-375 mx-auto bg-linear-to-r bg-primary m-3 rounded-2xl text-white pt-4 pb-4 px-4 relative overflow-hidden">
        <div className="max-w-10xl p-4 mx-auto">
          <div className="z-10 w-full lg:w-2/3">
            <p className="text-sm opacity-80 mb-2">Home / Courses</p>
            <h2 className="text-2xl md:text-4xl font-bold mb-4">Find Your Perfect Courses</h2>
        ...
        <section className="w-full lg:w-3/4">
          ...
          {initialCourses.length === 0 ? (
            ...
            initialCourses.map((course) => (
              <div key={course.id} className="p-6 border border-gray-200 m-4 rounded-xl shadow-sm bg-white">
                <h2 className="text-xl sm:text-2xl md:text-3xl font-medium md:font-medium bg-linear-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">
                  {course.name || 'N/A'} </h2>
                {/* Stats */}
                <div className="flex flex-wrap gap-8 mb-3 text-gray-500 font-medium">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-blue-600 text-md font-normal">₹</span>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm">
                      {course.avg_salary || 'N/A'} <span className="text-gray-400 font-medium">(Avg Salary)</span>
                    </span>
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center mb-3">
                  <h3 className="text-blue-600 font-medium text-xl">Colleges Offering this Course</h3>
                  <div className="flex gap-3">
                    <button className="w-10 h-10 rounded-full border border-blue-600 flex items-center justify-center text-blue-600 hover:bg-blue-600 hover:text-white">
                      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="flex grid-cols-3 md:grid-cols-3 sm:grid-cols-3 gap-6 overflow-x-scroll" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                  ...
                  {course.offered_at_colleges.map((item) => (
                    <div key={item.college.id} className="w-90 bg-white shrink-0 p-5 rounded-xl border border-blue-100 shadow-sm">
                      <div className="flex justify-between items-start mb-1">
                        <h4 className="font-medium text-gray-600 text-lg w-2/3">
                          {item.college.name}
                        </h4>
                        <Image src={item.college.logo_url || '/hero-corner.png'}
                          alt="Hero corner image"
                          width={600}
                          height={600} className="w-15 h-15 opacity-80" />
                      </div>
                      <p className="text-blue-500 text-sm font-medium mb-3">{item.college.city}</p>
                      <div className="flex gap-2">
                        <Link href={`/colleges/${item.college.slug}?courses&fees&courseId=${course.id}`} className="flex-1 py-2 border border-blue-600 text-blue-600 rounded-lg font-bold text-md text-center">View</Link>
                        <button className="px-6 py-2 bg-blue-600 text-white rounded-lg font-bold text-md shadow-md">Apply now</button>
                      </div>
                    </div>
                  ))}
                </div>
