    ...
    <div className="space-y-12">
      {allDepartments.map((dept, deptIndex) => (
        <div key={deptIndex} className="space-y-8">
          <h2 className="text-2xl font-bold text-slate-800 border-b-2 border-blue-600 pb-2">
            {dept.department}
          </h2>
          ...
          {dept.streams.map((stream, streamIndex) => (
            <div key={streamIndex} className="space-y-6">
              <div className="bg-slate-50 p-4 border-l-4 border-blue-600">
                ...
                <p className="text-sm text-slate-600 mt-1">Exam: {stream.exam}</p>
              </div>

              {stream.blocks.map((block, blockIndex) => (
                <div key={blockIndex} className="ml-6 space-y-4">
