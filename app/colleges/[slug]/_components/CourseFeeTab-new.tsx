    ...
    <div className="space-y-4">
      ...
      <div className="bg-white p-6 rounded-xl">
        ...
        <ul className="space-y-2 text-sm">
          <li><span className="text-blue-600 mt-1">•</span> <button onClick={() => document.getElementById('courses-overview')?.scrollIntoView({ behavior: 'smooth', block: 'start' })} className="hover:underline hover:text-blue-600 text-left">{collegeName} Courses & Fees Overview</button></li>
          <li><span className="text-blue-600 mt-1">•</span> <button onClick={() => document.getElementById('courses-table')?.scrollIntoView({ behavior: 'smooth', block: 'start' })} className="hover:underline hover:text-blue-600 text-left">{collegeName} Courses Table</button></li>
          <li><span className="text-blue-600 mt-1">•</span> <button onClick={() => document.getElementById('fee-structure')?.scrollIntoView({ behavior: 'smooth', block: 'start' })} className="hover:underline hover:text-blue-600 text-left">{collegeName} Fee Structure Overview</button></li>
          <li><span className="text-blue-600 mt-1">•</span> <button onClick={() => document.getElementById('all-courses')?.scrollIntoView({ behavior: 'smooth', block: 'start' })} className="hover:underline hover:text-blue-600 text-left">All Courses Offered in {collegeName}</button></li>
          <li><span className="text-blue-600 mt-1">•</span> <button onClick={() => document.getElementById('course-faqs')?.scrollIntoView({ behavior: 'smooth', block: 'start' })} className="hover:underline hover:text-blue-600 text-left">Courses And Fees FAQs</button></li>
          <li><span className="text-blue-600 mt-1">•</span> <button onClick={() => document.getElementById('popular-courses')?.scrollIntoView({ behavior: 'smooth', block: 'start' })} className="hover:underline hover:text-blue-600 text-left">Top Courses Offered at {collegeName}</button></li>
          <li><span className="text-blue-600 mt-1">•</span> <button onClick={() => document.getElementById('similar-colleges')?.scrollIntoView({ behavior: 'smooth', block: 'start' })} className="hover:underline hover:text-blue-600 text-left">More Similar Colleges</button></li>
        </ul>
      </div>

      {/* Overview Section */}
      <section id="courses-overview" className="bg-white p-8 rounded-2xl">
        <h2 className="text-xl font-bold mb-4">{collegeName} Courses & Fees 2026</h2>
        <p className="text-gray-600 leading-relaxed">{coursePara || `${collegeName} offers a wide range of undergraduate and postgraduate courses across various streams. The college provides quality education with experienced faculty and modern infrastructure.`}</p>
      </section>

      {/* Courses Table */}
      <section id="courses-table" className="bg-white p-8 rounded-2xl">
        <h3 className="text-lg font-bold mb-4">{collegeName} Courses Summary</h3>
        <div className="overflow-x-auto">
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-gray-300">
                <th className="text-left p-3 text-sm font-bold">Course Name</th>
                <th className="text-left p-3 text-sm font-bold">Duration</th>
                <th className="text-left p-3 text-sm font-bold">Type</th>
                <th className="text-right p-3 text-sm font-bold">Fees</th>
              </tr>
            </thead>
            <tbody>
              {courseOfferings.slice(0, 10).map((offering) => (
                <tr key={offering.id} className="border-b border-gray-300 last:border-b-0 hover:bg-gray-50">
                  <td className="p-3 font-medium text-gray-800">{offering.course.name}</td>
      ...
      <section id="fee-structure" className="bg-white p-8 rounded-2xl">
        <h3 className="text-xl font-bold mb-4">{collegeName} Fee Structure Overview</h3>
      ...
      <section id="all-courses" className="bg-white p-8 rounded-2xl">
        <h3 className="text-xl font-bold mb-6">All Courses at {collegeName}</h3>
        <div className="flex items-center gap-2 mb-4">
          <Filter size={18} className="text-blue-600" />
          <h4 className="font-bold text-md">Filter Courses</h4>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {filteredCourses.map((offering) => (
            <div key={offering.id} className="border border-gray-200 rounded-xl p-5 hover:border-blue-200 hover:shadow-sm transition">
              <h4 className="font-bold text-md text-gray-800 mb-3">{offering.course.name}</h4>
              ...
              <button
                ...
                className="block w-full text-center py-2.5 bg-blue-600 text-white rounded-lg text-sm font-bold hover:bg-blue-700 transition"
              >
                View Details
              </button>
            </div>
